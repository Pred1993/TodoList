...
const Task = React.memo((props: TaskPropsType) => {
    const onclickHandler = useCallback(() => { // Функция-обработчик для вызова callback-функции удаления тасок
        props.removeTasks(props.todoListId, props.task.id)
    }, [props.removeTasks, props.todoListId, props.task.id])

    const onChangeCheckedHandler = useCallback((e: ChangeEvent<HTMLInputElement>) => { // Функция-обработчик для вызова callback-функции изменения чекеда
